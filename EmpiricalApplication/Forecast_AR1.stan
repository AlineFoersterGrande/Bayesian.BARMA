data {  // AR sem coeficientes e prioris normais para phi 
    /* Outcome data */ 
     int<lower=0> N;
     int<lower=0> h;
 ...
 generated quantities { 
   ...
   vector[h] mup;
   vector[h] pred;
   ar_partp[1] = logity[N]; // g(y_{t-1})

  for(t in 1:h){
     eta_tp[t] = alpha + phi[1]*ar_partp[t]; // phi[1] e theta[1] são os únicos phi e theta que foram estimados acima (modelo BARMA(1,1))
     mup[t] = inv_logit(eta_tp[t]);
     if (mup[t] < 0.0001) mup[t] = 0.0001;
     if (mup[t] > 0.9999) mup[t] = 0.9999;
     
     pred[t] = beta_rng(nu*mup[t], (1-mup[t])*nu);
     ar_partp[t+1] = logit(pred[t]);
   }
 }
