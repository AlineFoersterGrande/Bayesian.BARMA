data {  // ARMA sem coeficientes e prioris normais para phi e theta
 ...
 generated quantities { 
  ...
  for(t in 1:h){
     eta_tp[t] = alpha + phi[1]*ar_partp[t]+theta[1]*rtp[t]; // phi[1] e theta[1] são os únicos phi e theta que foram estimados acima (modelo BARMA(1,1))
     mup[t] = inv_logit(eta_tp[t]);
     if (mup[t] < 0.0001) mup[t] = 0.0001;
    // if (mup[t] > 0.9999) mup[t] = 0.9999;
     
     pred[t] = beta_rng(nu*mup[t], (1-mup[t])*nu);
     rtp[t+1] = logit(pred[t]) - eta_tp[t];
